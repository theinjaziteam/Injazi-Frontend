...
const FAST_MODEL = 'gemini-2.5-flash';  // Fast responses
const SMART_MODEL = 'gemini-2.5-pro';   // Complex tasks
...
export async function generateDailyTasks(goal: Goal, day: number, userProfile: string, checkIn: string, pending: any[]): Promise<Task[]> {
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            [{ 
              "title": "Short energetic title (3-5 words)", 
              "description": "One clear sentence instruction", 
...
export async function getChatResponse(
    ...
    if (!apiKey) {
        return "I'm your AI Guide. To enable full AI features, please add your Gemini API key to the app settings.";
    }
    ...
    try { 
        ...
        // Add new message with attachment if present
...
export async function generateGoalVisualization(title: string, category: string): Promise<string> { 
    // Using placeholder images - you could integrate DALL-E or other image APIs here
