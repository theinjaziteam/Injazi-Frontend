...
export default function BridgeHub({ isOpen, onClose }: BridgeHubProps) {
    ...
    return (
        <div 
            className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm modal-backdrop-bridge"
        ...
        >
            <div 
                className="bg-gray-50 w-full h-[95vh] sm:h-[90vh] sm:max-w-2xl sm:rounded-2xl rounded-t-[2rem] shadow-2xl flex flex-col overflow-hidden modal-content-bridge"
                onClick={(e) => e.stopPropagation()}
            >
                {/* ===== HEADER ===== */}
                <div className="flex-shrink-0 bg-[#171738] rounded-b-[2rem] shadow-xl relative overflow-hidden">
                    {/* Decorative Gradient */}
                    <div className="absolute top-0 right-0 w-64 h-64 bg-[#3423A6] rounded-full blur-[80px] opacity-20 translate-x-1/2 -translate-y-1/2 animate-float" />
                    ...
                    <div className="relative z-10 p-5 pt-safe">
                        ...
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button 
                                onClick={onClose}
                                className="p-2.5 bg-white/10 rounded-full backdrop-blur-md hover:bg-white/20 transition-colors focus-ring"
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    <div className="flex items-center gap-2 bg-[#DFF3E4]/20 px-3 py-1 rounded-full">
                                        <div className="w-2 h-2 bg-[#DFF3E4] rounded-full status-dot-connected"/>
                                    ...
                                    {expiredCount > 0 && (
                                        <div className="flex items-center gap-2 bg-amber-400/20 px-3 py-1 rounded-full">
                                            <div className="w-2 h-2 bg-amber-400 rounded-full status-dot-expired"/>
                        ...
                        <div className="mt-4 relative">
                            ...
                            <input
                                ...
                                className="search-input w-full bg-white/10 border border-white/10 rounded-xl py-3 pl-11 pr-10 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#3423A6]/50 focus:bg-white/15 transition-all"
                ...
                {/* ===== CATEGORY TABS ===== */}
                <div className="flex-shrink-0 bg-gray-50 border-b border-gray-100 category-scroll-container">
                    <div 
                        className="category-scroll flex gap-2 px-5 py-3 overflow-x-auto"
                    >
                        {CATEGORIES.map((cat) => (
                            <button
                                key={cat.id}
                                onClick={() => setActiveCategory(cat.id)}
                                className={`category-pill flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all ${
                                    activeCategory === cat.id
                                        ? 'category-pill-active bg-[#171738] text-white shadow-lg'
                ...
                >
                    {isLoading ? (
                        ...
                        <div className="empty-state animate-fade-in">
                            ...
                            <h3 className="empty-state-title">No platforms found</h3>
                            ...
                            {searchQuery && (
                                <button 
                                    onClick={() => setSearchQuery('')}
                                    className="empty-state-action mt-4 px-4 py-2 bg-[#3423A6] text-white rounded-xl text-sm font-bold hover:bg-[#171738] transition-colors"
                                >
                                    Clear Search
                                </button>
                            )}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 animate-stagger">
                            {filteredPlatforms.map((platform) => {
                                ...
                                return (
                                    <div
                                        key={platform.id}
                                        className={`platform-card relative bg-white rounded-2xl p-4 border-2 transition-all group ${
                                            status === 'connected' 
                                                ? 'border-[#DFF3E4] connected-glow' 
                                                : status === 'expired'
                                                ? 'border-amber-300'
                                    ...
                                    >
                                        {/* Status Badge */}
                                        {status === 'connected' && (
                                            <div className="absolute -top-2 -right-2 w-5 h-5 bg-[#DFF3E4] rounded-full flex items-center justify-center shadow-lg animate-bounce-in">
                                                <Icons.Check className="w-3 h-3 text-[#171738]"/>
                                            </div>
                                        )}
                                        {status === 'expired' && (
                                            <div className="absolute -top-2 -right-2 w-5 h-5 bg-amber-400 rounded-full flex items-center justify-center shadow-lg animate-bounce-in">
                                                <Icons.AlertCircle className="w-3 h-3 text-white"/>
                                            </div>
                                        )}

                                        {/* Platform Icon */}
                                        <div className={`platform-icon w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-3 ${
                                        ...
                                        {!platform.configured ? (
                                            ...
                                            <button 
                                                ...
                                                className="connect-btn w-full py-2 bg-gray-100 text-gray-500 rounded-xl text-xs font-bold hover:bg-red-50 hover:text-red-500 transition-colors"
                                            >
                                                Disconnect
                                            </button>
                                        ) : status === 'expired' ? (
                                            <button 
                                                ...
                                                className="connect-btn w-full py-2 bg-amber-100 text-amber-700 rounded-xl text-xs font-bold hover:bg-amber-200 transition-colors flex items-center justify-center gap-1.5 disabled:opacity-50"
                                            >
                                                {isConnecting ? (
                                                    <>
                                            ...
                                            <button 
                                                ...
                                                disabled={isConnecting}
                                                className="connect-btn w-full py-2 bg-[#3423A6] text-white rounded-xl text-xs font-bold hover:bg-[#171738] transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
                                            >
                                                {isConnecting ? (
                                                    <>
                ...
                >
                    <div className="flex items-center justify-between">
                        ...
                        <button 
                            onClick={onClose}
                            className="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-xs font-bold hover:bg-gray-200 transition-colors focus-ring"
                        >
                            Done
                        </button>
                    </div>
                    ...
                    >
                        <div className="flex justify-between items-center mb-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 id="klaviyo-modal-title" className="text-lg font-black text-[#171738]">
                                        Connect Klaviyo
                                    </h2>
                            ...
                            <button 
                                ...
                                className="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors focus-ring"
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#3423A6]/50 focus:border-[#3423A6] transition-all"
                                    autoComplete="off"
                                />
                            </div>
                            <div>
                                <label 
                                    htmlFor="klaviyo-public-key"
                                    className="block text-xs font-bold text-gray-600 mb-1.5"
                                >
                        ...
                        <div className="mt-5 p-3 bg-blue-50 rounded-xl">
                            <div className="flex gap-2">
                                ...
                                <p className="text-xs text-blue-600">
                                    Find your API keys in Klaviyo under{' '}
                        ...
                        <div className="flex gap-3 mt-6">
                            <button
                                onClick={() => setShowKlaviyoModal(false)}
                                className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-bold text-sm hover:bg-gray-200 transition-colors focus-ring"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="flex-1 py-3 bg-[#3423A6] text-white rounded-xl font-bold text-sm hover:bg-[#171738] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 focus-ring"
                            >
            ...
            {notification && (
                <div 
                    className={`notification-toast fixed top-6 left-1/2 -translate-x-1/2 z-[120] px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 ${
                ...
                >
                    {notification.type === 'success' ? (
                        <div className="w-6 h-6 bg-[#171738]/10 rounded-full flex items-center justify-center">
