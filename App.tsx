...
function AppContent() {
    ...
    const showNav = ![AppView.ONBOARDING, AppView.LIVE_CALL, AppView.TASK_EXECUTION, AppView.SETTINGS, AppView.TASK_HISTORY, AppView.TASK_SELECTION].includes(view);

    return (
        <div className="h-[100dvh] w-screen overflow-hidden flex flex-col font-sans transition-colors duration-300" style={{ backgroundColor: 'var(--bg-primary)' }}>
            <div className="flex-1 overflow-hidden relative" id="main-container">
                ...
                {(view === AppView.PLANS || view === AppView.USER_PROFILE) && (
                    <div className="p-10 text-center flex flex-col items-center justify-center h-full">
                        <h1 className="text-2xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>View: {view}</h1>
                        <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>This section is under construction.</p>
                        <button onClick={() => setView(AppView.DASHBOARD)} className="px-6 py-3 rounded-full font-bold" style={{ backgroundColor: 'var(--accent)', color: 'var(--accent-dark)' }}>Back to Dashboard</button>
                    </div>
                )}
            </div>
            ...
            {showAdOverlay && (
                <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-6 animate-fade-in">
                    <div className="w-full max-w-sm rounded-3xl overflow-hidden relative" style={{ backgroundColor: 'var(--bg-card)' }}>
                        ...
                        <div className="p-6 text-center">
                            <h2 className="text-2xl font-black mb-4" style={{ color: 'var(--text-on-card)' }}>Upgrade Your Life</h2>
                        </div>
                    </div>
                </div>
            )}
